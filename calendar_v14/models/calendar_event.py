...
class Meeting(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("default_res_model") and not self.env.context.get(
            ...
            self = self.with_context(
                default_res_model_id=self.env["ir.model"]
                .sudo()
                .search(
                    [("model", "=", self.env.context["default_res_model"])], limit=1
                )
                .id
            )
        ...
        if (
            ...
            defaults["res_model_id"] = (
                self.env["ir.model"]
                .sudo()
                .search([("model", "=", self.env.context["active_model"])], limit=1)
                .id
            )
        if (
    ...
    def _compute_stop(self):
        ...
        for event in self:
            ...
            event.stop = event.start + timedelta(
    ...
    def _split_recurrence(self, time_values):
        ...
        return self._apply_recurrence_values(
            {previous_week_day_field: False, **self._get_recurrence_params(),},
            future=True,
        )
            ...
            if any(
                {
                    ...
                    for field in self.env["calendar.event"]._get_time_fields()
                    if field in values
                }
            ):
        ...
        if update_recurrence:
            if break_recurrence:
                ...
                if not update_start and (time_values or recurrence_values):
                    raise UserError(
                        _(
                            "Updating All Events is not allowed when dates or time is modified. You can only update one particular event and following events."
                        )
                    )
        ...
        for values in vals_list:
            ...
            if not values.get("activity_ids"):
                ...
                if not defaults.get("activity_ids") and res_model_id and res_id:
                    if hasattr(
                        self.env[
                            self.env["ir.model"].sudo().browse(res_model_id).model
                        ],
                        "activity_ids",
                    ):
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        def my_events(events):
            ...
            my = [
                event
                for event in events
                if event.get("user_id") and event.get("user_id")[0] == self.env.uid
            ]
            others = [
                event
                for event in events
                if not event.get("user_id") or event.get("user_id")[0] != self.env.uid
            ]
            return my, others
    ...
    def _search(
        ...
        return super(Meeting, self.with_context(virtual_id=False),)._search(
            args,
            offset=offset,
            limit=limit,
            order=order,
